...
def step2():
    st.subheader(f"2. What do you need about {st.session_state.category}?")
    cols = st.columns(2)
    ...
    user_query = st.text_input("Type your custom question about this insurance category:")
    if user_query and st.button("Ask AI"):
        st.session_state.subcategory = "Custom"
...
def step3():
    ...
    query = st.session_state.user_query if subcategory == "Custom" else SUBCATEGORIES[subcategory]
    query_vec = embed_model.encode([query])
    _, indices = faiss_index.search(query_vec, k=5)
    context = [chunks[i] for i in indices[0] if metadata[i] == category]

    # Fallback Detection
    expected_keywords = ["policy", "coverage", "claim", "premium", "types", "insurance"]
    is_complex = False
    ...
    if len(context) < 1:
        ...
        is_complex = True
    elif not any(word in query.lower() for word in expected_keywords):
        is_complex = True

    if is_complex:
        st.warning("⚠️ This might be a complex or unsupported query.")
        st.markdown("Please contact a human agent for assistance:\n\n📧 **support@insuranceai.com**\n📞 **+1-800-INSUREME**")
